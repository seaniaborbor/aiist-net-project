<form action="<?=base_url('/dashboard/edit/blog/'.$blog_data['id'])?>" method="post" enctype="multipart/form-data">
	<div class="form-group mb-3">
		<label>Post Title</label>
		<input type="text" name="title" value="<?=$blog_data['title']?>" class="form-control" >
	...
	<div class="form-group mb-3">
		...
		<input type="file" name="featureImg" required  class="form-control" >
	...
	<div class="form-group mb-3">
		...
		<textarea id="content" class="form-control" name="postbody" ><?=$blog_data['postbody']?></textarea>
		...
		<select class="form-control" name="category" >
			...
			<?php if(isset($all_categories)): ?>
				<?php foreach($all_categories as $cat): ?>
					<option <?=set_select('category', $cat['courseName'])?> value="<?=$cat['courseName']?>"><?=$cat['courseName']?></option>
				<?php endforeach;?>
			<?php endif; ?>
		</select>
